...
class SmdImporter(bpy.types.Operator, Logger):
    ...
    def applyFrames(self, keyframes: dict[bpy.types.PoseBone, list[KeyFrame]], num_frames: int):
        ...
        if smd.jobType == ANIM:
            ...
            # Action slots are a Blender 4.4+ feature; use plain actions on 4.1
    ...
    def readPolys(self):
        ...
        if countPolys:
            ...
            # Blender 4.1: normals_split_custom_set requires use_auto_smooth = True.
            # 4.2+ removed use_auto_smooth; guard with hasattr so the same file
            # works on both versions without any other changes.
